...
import type { EconomyLeaderboardProps, EconomyLeaderboardPropsType } from 'types/canvacord';
...
const fixed = (v: number, r: boolean) => {
  if (!r) return v;
  const formatter = new Intl.NumberFormat('en-US', { currency: 'USD', style: 'currency', notation: 'compact' });
  return formatter.format(v);
};

export class LeaderboardBuilder extends Builder<EconomyLeaderboardProps> {
  public variant?: LeaderboardVariants;

  /**
   * Create a new leaderboard ui builder
   */
  public constructor(props: EconomyLeaderboardPropsType) {
    super(600, 650);
    ...
    this.bootstrap({
      ...
      text: props.text ?? { bank: 'BANK: ', wallet: 'WALLET: ', rank: 'RANK: ' }
    });
   ...
   */
  public setHeader(data: EconomyLeaderboardProps['header'] & {}) {
   ...
   */
  public setPlayers(players: EconomyLeaderboardProps['players']) {
   ...
   */
  public setTextStyles(config: Partial<EconomyLeaderboardProps['text']>) {
   ...
   */
  public async renderDefaultTop({ avatar, displayName, bank, rank, username, wallet }: EconomyLeaderboardProps['players'][number]) {
    ...
    return JSX.createElement(
      ...
      JSX.createElement(
        ...
        JSX.createElement(
          ...
          `${this.options.get('text').bank} ${fixed(bank, this.options.get('abbreviate'))}` as unknown as JSX.Element
        ),
        JSX.createElement(
          'h4',
          { className: `text-sm text-[${currentColor}] m-0` },
          `${this.options.get('text').wallet} ${fixed(wallet, this.options.get('abbreviate'))}` as unknown as JSX.Element
        )
      )
    ) as JSX.Element;
  }
   ...
   */
  public async renderDefaultPlayer({ avatar, displayName, bank, rank, username, wallet }: EconomyLeaderboardProps['players'][number]) {
    return JSX.createElement(
      'div',
      {
      ...
      JSX.createElement(
        ...
        JSX.createElement(
          'h4',
          { className: 'text-[#9ca0a5] text-sm m-0' },
          this.options.get('text').bank as unknown as JSX.Element,
          JSX.createElement('span', { className: 'text-white ml-1' }, fixed(bank, this.options.get('abbreviate')) as unknown as JSX.Element)
        ),
        JSX.createElement(
          'h4',
          { className: 'text-[#9ca0a5] text-sm m-0' },
          this.options.get('text').wallet as unknown as JSX.Element,
          JSX.createElement('span', { className: 'text-white ml-1' }, fixed(wallet, this.options.get('abbreviate')) as unknown as JSX.Element)
        )
      )
    ) as JSX.Element;
  }
  ...
  public async renderHorizontalPlayer({ avatar, displayName, bank, rank, username, wallet }: EconomyLeaderboardProps['players'][number]) {
    return JSX.createElement(
      'div',
      {
