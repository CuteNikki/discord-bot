...
export default new Command({
  ...
  data: {
    name: 'support',
    description: 'Gives you an invite link to the support server',
  ...
  async execute({ interaction, client }) {
    ...
    await interaction.editReply({
      embeds: [new EmbedBuilder().setColor(Colors.Blurple).setTitle(i18next.t('support.title', { lng })).setDescription(settings.support.inviteUrl)],
    });
  },
});
