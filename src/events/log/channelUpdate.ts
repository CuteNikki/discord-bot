...
export default new Event({
  ...
  async execute(client, oldChannel, newChannel) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({ name: t('log.channelUpdate.channel', { lng }), value: `${newChannel.toString()} (\`${newChannel.name}\` | ${newChannel.id})` })
      .setTimestamp();
    ...
    if (newChannel.name !== oldChannel.name)
      embed.addFields(
        { name: t('log.channelUpdate.old_name', { lng }), value: `\`${oldChannel.name}\``, inline: true },
        { name: t('log.channelUpdate.new_name', { lng }), value: `\`${newChannel.name}\``, inline: true },
        emptyField
      );
    if (newChannel.position !== oldChannel.position)
      embed.addFields(
        { name: t('log.channelUpdate.old_position', { lng }), value: `${oldChannel.position}`, inline: true },
        { name: t('log.channelUpdate.new_position', { lng }), value: `${newChannel.position}`, inline: true },
        emptyField
      );
    if (newChannel.type !== oldChannel.type)
      embed.addFields(
        { name: t('log.channelUpdate.old_type', { lng }), value: `${ChannelType[oldChannel.type]}`, inline: true },
        { name: t('log.channelUpdate.new_type', { lng }), value: `${ChannelType[newChannel.type]}`, inline: true },
        emptyField
      );
    ...
    if (
      ...
      if (newChannel.topic !== oldChannel.topic)
        embed.addFields(
          { name: t('log.channelUpdate.old_topic', { lng }), value: `${oldChannel.topic?.slice(0, 1000) || '/'}`, inline: true },
          { name: t('log.channelUpdate.new_topic', { lng }), value: `${newChannel.topic?.slice(0, 1000) || '/'}`, inline: true },
          emptyField
        );
      if (newChannel.nsfw !== oldChannel.nsfw)
        embed.addFields(
          { name: t('log.channelUpdate.old_nsfw', { lng }), value: `${newChannel.nsfw}`, inline: true },
          { name: t('log.channelUpdate.new_nsfw', { lng }), value: `${oldChannel.nsfw}`, inline: true },
          emptyField
        );
      if ((newChannel.rateLimitPerUser || 0) !== (oldChannel.rateLimitPerUser || 0))
        embed.addFields(
          { name: t('log.channelUpdate.old_rate_limit', { lng }), value: `${oldChannel.rateLimitPerUser || '0'}s`, inline: true },
          { name: t('log.channelUpdate.new_rate_limit', { lng }), value: `${newChannel.rateLimitPerUser || '0'}s`, inline: true },
          emptyField
        );
    ...
    if (
      ...
      if (newChannel.bitrate !== oldChannel.bitrate)
        embed.addFields(
          { name: t('log.channelUpdate.old_bitrate', { lng }), value: `${oldChannel.bitrate}`, inline: true },
          { name: t('log.channelUpdate.new_bitrate', { lng }), value: `${newChannel.bitrate}`, inline: true },
          emptyField
        );
      if (newChannel.rtcRegion !== oldChannel.rtcRegion)
        embed.addFields(
          { name: t('log.channelUpdate.old_region', { lng }), value: `${oldChannel.rtcRegion || 'Automatic'}`, inline: true },
          { name: t('log.channelUpdate.new_region', { lng }), value: `${newChannel.rtcRegion || 'Automatic'}`, inline: true },
          emptyField
        );
      if (newChannel.userLimit !== oldChannel.userLimit)
        embed.addFields(
          { name: t('log.channelUpdate.old_user_limit', { lng }), value: `${oldChannel.userLimit}`, inline: true },
          { name: t('log.channelUpdate.new_user_limit', { lng }), value: `${newChannel.userLimit}`, inline: true },
          emptyField
        );
      if (newChannel.videoQualityMode !== oldChannel.videoQualityMode)
        embed.addFields(
          { name: t('log.channelUpdate.old_video_quality', { lng }), value: `${VideoQualityMode[oldChannel.videoQualityMode || 1]}`, inline: true },
          { name: t('log.channelUpdate.new_video_quality', { lng }), value: `${VideoQualityMode[newChannel.videoQualityMode || 1]}`, inline: true },
          emptyField
        );
    }
